...
import boto3
import pytest
from moto import mock_apigateway, mock_dynamodb, mock_sqs
...
class TestAWSDynamoDBRepository:
    ...
    def setup_method(self):
        ...
        self.repository = AWSDynamoDBRepository(table_name="test-table", region="us-east-1")
    ...
    async def test_save_search_result_success(self):
        ...
        result = SearchResult(
            request_id="req-123",
            address=Address(street="Test Street", city="Berlin", postal_code="10115", country="Germany"),
            offers=[],
        ...
        response = self.table.get_item(Key={"pk": "SEARCH_RESULT", "sk": "req-123"})
    ...
    async def test_save_connection_success(self):
        ...
        # Verify item was saved
        response = self.table.get_item(Key={"pk": "CONNECTION", "sk": "conn-123"})
        assert "Item" in response
    ...
    async def test_get_active_connections(self):
        ...
        connections = [
            {"pk": "CONNECTION", "sk": "conn-1", "connection_id": "conn-1", "user_id": "user-1", "status": "active"},
            {"pk": "CONNECTION", "sk": "conn-2", "connection_id": "conn-2", "user_id": "user-2", "status": "active"},
            {"pk": "CONNECTION", "sk": "conn-3", "connection_id": "conn-3", "user_id": "user-3", "status": "disconnected"},
        ]
...
class TestAWSSQSAdapter:
    ...
    def setup_method(self):
        ...
        self.adapter = AWSSQSAdapter(queue_url=self.queue_url, region="us-east-1")
...
class TestAWSWebSocketAdapter:
    ...
    def setup_method(self):
        ...
        self.adapter = AWSWebSocketAdapter(
            api_endpoint="https://test.execute-api.us-east-1.amazonaws.com/test", region="us-east-1"
        )
    ...
    async def test_send_message_to_connection_success(self, mock_boto3):
        ...
        success = await self.adapter.send_message_to_connection(connection_id="conn-123", message=message)
    ...
    async def test_send_message_connection_gone(self, mock_boto3):
        ...
        success = await self.adapter.send_message_to_connection(connection_id="conn-gone", message=message)
