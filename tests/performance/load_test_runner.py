...
class LoadTestRunner:
    ...
    async def run_async_load_test(self, async_target_function: Callable) -> LoadTestResult:
        ...
        logger.info(f"Starting async load test with {self.config.concurrent_users} users, "
        ...
        for user_id in range(self.config.concurrent_users):
            for request_id in range(self.config.requests_per_user):
                task = self._create_async_task(
                    semaphore, async_target_function, user_id, request_id
                )
                tasks.append(task)
    ...
    def run_sync_load_test(self, sync_target_function: Callable) -> LoadTestResult:
        ...
        logger.info(f"Starting sync load test with {self.config.concurrent_users} users, "
                   f"{self.config.requests_per_user} requests per user")
            ...
            for user_id in range(self.config.concurrent_users):
                for request_id in range(self.config.requests_per_user):
                    future = executor.submit(
                        self._execute_sync_request, sync_target_function, user_id, request_id
                    )
    ...
    async def _create_async_task(self, semaphore: asyncio.Semaphore, 
                                target_function: Callable, user_id: int, request_id: int):
