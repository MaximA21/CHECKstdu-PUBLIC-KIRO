...
class TestWebSocketConnectionLimits:
    ...
    def connect_handler(self, connection_management_use_case, mock_logger):
        ...
        return ConnectHandler(
            connection_management_use_case=connection_management_use_case,
            logger=mock_logger
        )
    ...
    @pytest.mark.asyncio
    async def test_result_count_increment_and_limit_check(
        self, 
    ...
    @pytest.mark.asyncio
    async def test_time_limit_enforcement(
        self, 
        connection_management_use_case, 
        mock_connection_repository
    ):
...
class TestWebSocketResultDeliveryAndTimeout:
    ...
    async def test_result_delivery_increments_count(
        self, 
        results_handler, 
        mock_connection_management_use_case,
        sample_results_event
    ):
        ...
        results_handler._process_results_use_case._search_result_repository.get_result_by_request_id = AsyncMock(return_value=None)
        ...
        results_handler._process_results_use_case._provider_registry.get_provider = AsyncMock(return_value=mock_provider_service)
    ...
    async def test_websocket_server_connection_tracking(self):
        ...
        server = WebSocketServerController(
            connection_management_use_case=mock_connection_use_case,
            logger=mock_logger
        )
        ...
        mock_connection_use_case.handle_connect.return_value = {
    ...
    async def test_connection_routing_with_implementation_version(self):
        ...
        with patch.dict('os.environ', {'IMPLEMENTATION_VERSION': 'old'}):
            connect_handler = ConnectHandler(
                connection_management_use_case=connection_use_case,
                logger=mock_logger
            )
            
            event = {
                "requestContext": {
                    "connectionId": "old_impl_conn_123",
                    "routeKey": "$connect"
                }
            }
        ...
        with patch.dict('os.environ', {'IMPLEMENTATION_VERSION': 'new'}):
            ...
            event = {
                "requestContext": {
                    "connectionId": "new_impl_conn_123",
                    "routeKey": "$connect"
                }
            }
    ...
    async def test_connection_routing_preserves_limits(self):
        ...
        for impl_version in ['old', 'new']:
            with patch.dict('os.environ', {'IMPLEMENTATION_VERSION': impl_version}):
                ...
                event = {
                    "requestContext": {
                        "connectionId": f"{impl_version}_conn_123",
                        "routeKey": "$connect"
                    }
                }
