...
async def simulate_provider_service(provider_name: str):
    ...
    if failure_type < 0.1:  # 10% critical failures
        ...
        raise ProviderUnavailableException(f"Provider {provider_name} temporarily unavailable", provider_name=provider_name)
    elif failure_type < 0.4:  # 10% validation errors
        raise InvalidAddressException("Invalid address format", address="123 Invalid St")

    # Success case
    return {"provider": provider_name, "offers": [{"speed": "100Mbps", "price": 29.99}, {"speed": "500Mbps", "price": 49.99}]}


async def fallback_provider_service(provider_name: str):
    """Fallback service that returns cached or default data."""
    return {"provider": f"{provider_name}_fallback", "offers": [{"speed": "50Mbps", "price": 19.99, "note": "Fallback offer"}]}
...
async def main():
    ...
    error_handler = initialize_error_handler(
        logger=logger, enable_monitoring=True, enable_circuit_breakers=True, enable_retry=True, enable_health_checks=True
    )

    # Register health checks
    if error_handler.health_check_registry:
        # Database health check
        db_health_check = DatabaseHealthCheck("database", simulate_database_connection, timeout_seconds=3.0, logger=logger)
        error_handler.health_check_registry.register(db_health_check)

        # External service health check
        service_health_check = ExternalServiceHealthCheck(
            "external_api", simulate_external_service_call, degraded_threshold_ms=1000.0, timeout_seconds=5.0, logger=logger
        )
        error_handler.health_check_registry.register(service_health_check)

        # Memory health check
        memory_health_check = MemoryHealthCheck(
            "memory", warning_threshold_percent=70.0, critical_threshold_percent=90.0, logger=logger
        )
        ...
        for provider in providers:
            try:
                ...
                result = await error_handler.execute_with_protection(
                    ...
                    context={"request_id": f"req_{i+1}_{provider}", "user_id": "user_123"},
                    enable_retry=True,
                ...
                error_response = error_handler.handle_error(
                    e, context={"request_id": f"req_{i+1}_{provider}", "user_id": "user_123"}, service_name=provider
                )
    ...
    test_errors = [
        ...
        ExternalServiceException("Test external service error", service_name="test_service", status_code=503),
        Exception("Test unexpected error"),
    ]

    for error in test_errors:
        response = error_handler.handle_error(error, context={"request_id": "test_req", "operation": "test_operation"})
