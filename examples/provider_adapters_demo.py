...
byteme_cable_100,ByteMe,100,4999,Cable"""
    ...
    for offer in offers:
        print(
            f"  - {offer.product_id}: {offer.speed_mbps} Mbps for {offer.monthly_cost_euros}€/month ({offer.connection_type})"
        )
            ...
            "offers": [
                {
                    ...
                    "productInfo": {"speed": 1000, "connectionType": "Fiber"},
                    "pricingDetails": {"monthlyCostInCent": 5999},
                }
            ],
        },
        {
            ...
            "offers": [
                {
                    "providerName": "PingPerfect",
                    "productInfo": {"speed": 100, "connectionType": "Cable"},
                    "pricingDetails": {"monthlyCostInCent": 3999},
                }
            ],
        },
    ]
...
async def demonstrate_aggregation():
    ...
    for i, offer in enumerate(sorted_by_price[:3], 1):
        print(
            f"  {i}. {offer.provider_name} - {offer.monthly_cost_euros}€/month ({offer.speed_mbps} Mbps, {offer.connection_type})"
        )
    ...
    for i, offer in enumerate(sorted_by_speed[:3], 1):
        print(
            f"  {i}. {offer.provider_name} - {offer.speed_mbps} Mbps ({offer.monthly_cost_euros}€/month, {offer.connection_type})"
        )
...
async def main():
    ...
    # Create test address
    address = MockAddress(street="Musterstraße", house_number="42", city="Berlin", postal_code="10115", country="DE")
