...
class UserService:
    ...
    def create_user(self, user_data: dict) -> str:
        ...
        try:
            ...
            self._logger.error("Failed to create user", 
                             {"email": user_data.get("email")}, 
                             exception=e)
            raise
...
class OrderService:
    ...
    def process_order(self, order_data: dict) -> str:
        ...
        if hasattr(self._logger, 'log_event'):
            self._logger.log_event("order_processing_started", {
        ...
        try:
            ...
            if hasattr(self._logger, 'log_performance'):
                self._logger.log_performance("order_validation", 25.3, True, {
                    "order_id": order_id,
                    "validation_rules": 5
                })
            ...
            if hasattr(self._logger, 'log_external_api_call'):
                self._logger.log_external_api_call(
                    "payment_service", "/api/charge", "POST", 
                    150.2, 200, True
                )
            
            if hasattr(self._logger, 'log_event'):
                self._logger.log_event("order_processing_completed", {
                    "order_id": order_id,
            ...
            self._logger.error("Failed to process order", 
...
def configure_logging_di_container() -> DIContainer:
    ...
    container.register(
        "user_service_logger",
        lambda: logger_factory.create_logger("services.user", LogLevel.DEBUG)
    )
    
    container.register(
        "order_service_logger", 
        lambda: logger_factory.create_structured_logger("services.order", LogLevel.INFO)
    )
    
    # Register services with logger dependencies
    container.register(
        UserService,
        lambda: UserService(container.resolve("user_service_logger"))
    )
    
    container.register(
        OrderService,
        lambda: OrderService(container.resolve("order_service_logger"))
    )
...
def demonstrate_lambda_handler_pattern():
    ...
    def lambda_handler(event, context):
        ...
        lambda_logger.info("Lambda execution started", {
        ...
        try:
            ...
            if event.get("action") == "create_user":
                ...
                lambda_logger.warning("Unknown action requested", {
                    "action": event.get("action")
                })
                return {"statusCode": 400, "body": {"error": "Unknown action"}}
        
        except Exception as e:
            lambda_logger.error("Lambda execution failed", 
                              {"event": event}, 
                              exception=e)
