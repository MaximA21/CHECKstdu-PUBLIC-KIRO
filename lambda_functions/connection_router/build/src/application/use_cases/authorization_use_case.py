...
class AuthorizationUseCase:
    ...
    async def authorize_request(
        ...
        """
        try:
            self._logger.info("Processing authorization request", {
            ...
            if address:
                self._logger.debug("Validating address", {
                    "address": address.full_address
                })
                
                if not await self._validate_address(address):
                    self._logger.info("Authorization denied: Invalid address", {
                        "address": address.full_address
                    })
            ...
            self._logger.error("Authorization error", {
                "error": str(e)
            }, exception=e)
    ...
    async def _validate_address(self, address: Address) -> bool:
        ...
        try:
            ...
            formatted_address = f"{address.street} {address.house_number}, {address.postal_code} {address.city}, {address.country}"
            ...
            self._logger.debug("Google Maps API response", {
                "status": status,
            ...
            if status == "OK" and results_count > 0:
                ...
                self._logger.info("Address validation failed", {
                    "status": status,
                    "results_count": results_count
                })
                return False
            ...
            self._logger.error("Address validation error", {
                "address": address.full_address,
                "error": str(e)
            }, exception=e)
