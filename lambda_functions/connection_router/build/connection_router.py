...
def lambda_handler(event, context):
    ...
    try:
        ...
        if not connection_id:
            ...
            return {
                'statusCode': 400,
                'body': json.dumps({'error': 'Connection ID missing'})
            }
...
def route_connect_request(event: Dict[str, Any], implementation: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            'statusCode': 500,
            'body': json.dumps({'error': 'Failed to route connect request'})
        }
...
def route_search_request(event: Dict[str, Any], implementation: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            'statusCode': 500,
            'body': json.dumps({'error': 'Failed to route search request'})
        }
