...
def lambda_handler(event, context):
    ...
    try:
        ...
        if event.get('warmer'):
            return {
                ...
                "body": json.dumps({
                    ...
                    "function": "connection_limit_enforcer"
                })
            }
        ...
        enforcement_result = asyncio.run(
            connection_management_use_case.enforce_connection_limits_for_all()
        )
        
        # Log results
        logger.info("Connection limit enforcement completed", {
        ...
        return {
            "statusCode": 500,
            "body": json.dumps({
                "error": "Connection limit enforcement failed",
                "message": str(e)
            })
        }
...
def cleanup_handler(event, context):
    ...
    try:
        ...
        cleanup_result = asyncio.run(
            connection_management_use_case.cleanup_stale_connections(timeout_minutes=30)
        )
