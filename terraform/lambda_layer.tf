...
resource "aws_lambda_layer_version" "json_layer_arm64" {
  count            = var.skip_lambda_package_validation ? 0 : (fileexists("../lambda_packages/json_layer_arm64.zip") ? 1 : 0)
  filename         = "../lambda_packages/json_layer_arm64.zip"
  layer_name       = "${var.project_name}-${var.environment}-json-arm64"
  source_code_hash = var.skip_lambda_package_validation ? "dummy" : filebase64sha256("../lambda_packages/json_layer_arm64.zip")
...
resource "aws_lambda_layer_version" "polars_layer_arm64" {
  count            = var.skip_lambda_package_validation ? 0 : (fileexists("../lambda_packages/polars_layer_arm64.zip") ? 1 : 0)
  filename         = "../lambda_packages/polars_layer_arm64.zip"
  layer_name       = "${var.project_name}-${var.environment}-polars-arm64"
  source_code_hash = var.skip_lambda_package_validation ? "dummy" : filebase64sha256("../lambda_packages/polars_layer_arm64.zip")
...
resource "aws_lambda_layer_version" "shared_dependencies_layer_arm64" {
  count            = var.skip_lambda_package_validation ? 0 : (fileexists("../lambda_packages/shared_dependencies_arm64.zip") ? 1 : 0)
  filename         = "../lambda_packages/shared_dependencies_arm64.zip"
  layer_name       = "${var.project_name}-${var.environment}-shared-deps-arm64"
  source_code_hash = var.skip_lambda_package_validation ? "dummy" : filebase64sha256("../lambda_packages/shared_dependencies_arm64.zip")
