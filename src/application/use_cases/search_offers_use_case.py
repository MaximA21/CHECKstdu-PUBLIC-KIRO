...
class SearchOffersUseCase:
    ...
    async def execute(
        ...
        """
        try:
            self._logger.info("Starting search offers use case", {
            ...
            self._logger.info("Search offers use case completed successfully", {
            ...
            self._logger.warning("Invalid address provided", {
                "address": address.full_address,
                "error": str(e)
            })
            raise
            ...
            self._logger.error("Search offers use case failed", {
                "connection_id": connection_id,
                "error": str(e)
            }, exception=e)
    ...
    async def _validate_address(self, address: Address) -> None:
        ...
        self._logger.debug("Address validation passed", {
            "address": address.full_address
        })
    ...
    async def _verify_connection(self, connection_id: str) -> ConnectionSession:
        ...
        self._logger.debug("Connection verified", {
            "connection_id": connection_id,
            "status": connection_session.status.value
        })
        
        return connection_session
    
    async def _update_connection_with_search(
        self, 
        ...
        self._logger.debug("Connection updated with search info", {
            "connection_id": connection_session.connection_id,
            "request_id": search_result.request_id
        })
    
    async def _queue_search_request(
        self, 
        search_result: SearchResult, 
        connection_id: str
    ) -> None:
        ...
        message_id = await self._message_queue.send_message(
            queue_name="search_requests",
            message=message,
            priority=MessagePriority.NORMAL
        )
        
        self._logger.info("Search request queued", {
            "request_id": search_result.request_id,
