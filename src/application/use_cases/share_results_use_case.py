...
class ShareResultsUseCase:
    ...
    async def execute(self, share_token: str) -> Dict[str, Any]:
        ...
        try:
            self._logger.info("Starting share results use case", {
            ...
            self._logger.info("Share results use case completed", {
                "share_token": share_token,
            ...
            processing_time = (time.time() - start_time) * 1000
            self._logger.error("Share results use case failed", {
                "share_token": share_token,
                "processing_time_ms": round(processing_time, 1),
                "error": str(e)
            }, exception=e)
    ...
    async def get_share_statistics(self, share_token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self._logger.error("Failed to get share statistics", {
                "share_token": share_token,
                "error": str(e)
            }, exception=e)
    ...
    async def extend_share_expiration(self, share_token: str, days: int) -> Dict[str, Any]:
        ...
        try:
            ...
            self._logger.info("Share expiration extended", {
                "share_token": share_token,
            ...
            self._logger.error("Failed to extend share expiration", {
                "share_token": share_token,
                "days": days,
                "error": str(e)
            }, exception=e)
    ...
    async def _get_search_result(self, share_token: str) -> SearchResult:
        ...
        if not search_result:
            self._logger.warning("Share token not found", {
                "share_token": share_token
            })
    ...
    async def _check_expiration(self, search_result: SearchResult) -> None:
        ...
        if search_result.is_expired:
            self._logger.warning("Attempted access to expired share", {
    ...
    async def _prepare_shared_data(self, search_result: SearchResult) -> Dict[str, Any]:
        ...
        sorted_offers = sorted(
            available_offers,
            key=lambda offer: offer.calculate_monthly_cost_per_mbps()
        )
        ...
        best_offers = {
            "cheapest": self._offer_to_dict(search_result.get_cheapest_offer()) if search_result.get_cheapest_offer() else None,
            "fastest": self._offer_to_dict(search_result.get_fastest_offer()) if search_result.get_fastest_offer() else None,
            "best_value": self._offer_to_dict(search_result.get_best_value_offer()) if search_result.get_best_value_offer() else None
        }
    ...
    async def _log_share_access(self, search_result: SearchResult, shared_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            search_result.add_metadata(access_key, {
            ...
            self._logger.debug("Share access logged", {
                "share_token": search_result.share_token,
                "request_id": search_result.request_id,
                "offers_shared": len(shared_data["offers"])
            })
            ...
            self._logger.warning("Failed to log share access", {
                "share_token": search_result.share_token,
                "error": str(e)
            })