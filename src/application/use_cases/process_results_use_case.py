...
class ProcessResultsUseCase:
    ...
    async def execute(
        ...
        try:
            ...
            search_result = await self._get_or_create_search_result(
            ...
            self._logger.info("Process results use case completed", {
                "request_id": request_id,
                "provider_name": provider_name,
            ...
            processing_time = (time.time() - start_time) * 1000
            self._logger.error("Process results use case failed", {
                "request_id": request_id,
                "provider_name": provider_name,
                "processing_time_ms": round(processing_time, 1),
                "error": str(e)
    ...
    async def _get_or_create_search_result(
        ...
        if search_result:
            self._logger.debug("Found existing search result", {
                "request_id": request_id,
                "share_token": search_result.share_token
            })
        ...
        self._logger.debug("Created new search result", {
            "request_id": request_id,
            "share_token": share_token
        })
    ...
    async def _process_successful_response(
        ...
        offers = await self._parse_provider_offers(
            provider_service, raw_response, search_result.address
        )
        ...
        for offer in offers:
            try:
                ...
                self._logger.warning("Skipped duplicate offer", {
                    "provider_name": provider_name,
        ...
        self._logger.info("Successfully processed provider response", {
            "provider_name": provider_name,
    ...
    async def _process_failed_response(
        ...
        self._logger.warning("Provider response failed", {
            "provider_name": provider_name,
            "error": error_message,
            "metadata": metadata
        })
    ...
    async def _parse_provider_offers(
        self,
        provider_service,
        raw_response: Any,
        address: Address
        ...
        try:
            ...
            self._logger.error("Failed to parse provider offers", {
                "provider_name": provider_service.provider_name,
                "error": str(e)
            }, exception=e)
    ...
    async def _notify_connection(
        ...
        if success:
            self._logger.debug("Notification sent to connection", {
                "connection_id": connection_id,
                "request_id": request_id,
                "provider_name": provider_name
            })
        else:
            self._logger.warning("Failed to send notification to connection", {
                "connection_id": connection_id,
                "request_id": request_id,
                "provider_name": provider_name
            })
