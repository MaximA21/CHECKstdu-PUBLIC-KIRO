...
class AuthorizerHandler(BaseController):
    ...
    def __init__(self, authorization_use_case: AuthorizationUseCase, logger: ILogger):
    ...
    async def handle_request(self, event: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not query_params:
                ...
                return await self._authorization_use_case.authorize_request(token="", method_arn=event["methodArn"])

            # Extract token
            token = query_params.get("token", "")

            self._logger.debug(
                "Authorization request received", {"has_token": bool(token), "token_length": len(token) if token else 0}
            )
            ...
            if all(key in query_params for key in ["street", "houseNumber", "city", "postalCode"]):
                try:
                    ...
                    self._logger.debug("Address extracted for validation", {"address": address.full_address})

                except Exception as e:
                    self._logger.warning("Failed to create address from query parameters", {"error": str(e)})

            # Execute authorization use case
            result = await self._authorization_use_case.authorize_request(
                token=token, method_arn=event["methodArn"], address=address
            )

            # Log success
            effect = result.get("policyDocument", {}).get("Statement", [{}])[0].get("Effect", "Unknown")
            self._log_request_success("Authorization", {"effect": effect, "has_address": address is not None}, start_time)
            ...
            return await self._authorization_use_case.authorize_request(token="", method_arn=event.get("methodArn", "*"))
            ...
            return {
                ...
                "policyDocument": {
                    "Version": "2012-10-17",
                    "Statement": [{"Action": "execute-api:Invoke", "Effect": "Deny", "Resource": event.get("methodArn", "*")}],
                },
...
def lambda_handler(event, context):
    ...
    authorization_use_case = AuthorizationUseCase(
        logger=container.get_logger("authorization_use_case"), google_maps_api_key=google_maps_api_key
    )
