    ...
    async def handle_request(self, event: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await self._connection_management_use_case.handle_disconnect(
                connection_id=connection_id, reason=disconnect_reason
            )
            ...
            self._log_request_success(
                ...
                {"connection_id": connection_id, "duration_seconds": result.get("duration_seconds", 0)},
                start_time,
            )

            return self._create_success_response(
                {"message": "Getrennt", "connection_id": connection_id, "status": result["status"]}
            )
