...
class WebSocketServerController(BaseController):
    ...
    async def handle_connection(self, websocket, path: str) -> None:
        ...
        try:
            ...
            await self._connection_management_use_case.handle_connect(
                ...
                metadata={"path": path, "connected_at": connection.connected_at.isoformat()}
            )
            ...
            self._logger.error("WebSocket connection error", {
                "connection_id": connection_id,
                "error": str(e)
            }, exception=e)
        finally:
        ...
        try:
            async for message in connection.websocket:
                try:
                    ...
                    self._logger.error("Message processing error", {
            ...
            self._logger.error("Message handling error", {
                "connection_id": connection.connection_id,
                "error": str(e)
            }, exception=e)
    
    async def _process_message(self, connection: WebSocketConnection, message_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            if message_type == "ping":
                ...
                if topic:
                    ...
                    return {"type": "error", "message": "Topic required for subscription", "timestamp": datetime.utcnow().isoformat()}
                ...
                if topic:
                    ...
                    result = await self._connection_management_use_case.unsubscribe_from_updates(connection.connection_id, topic)
                    return {"type": "unsubscription_confirmed", "data": result, "timestamp": datetime.utcnow().isoformat()}
                else:
                    return {"type": "error", "message": "Topic required for unsubscription", "timestamp": datetime.utcnow().isoformat()}
            
            else:
                # Unknown message type
                self._logger.warning("Unknown WebSocket message type", {
                    "connection_id": connection.connection_id,
                    "message_type": message_type
                })
                return {"type": "error", "message": f"Unknown message type: {message_type}", "timestamp": datetime.utcnow().isoformat()}
    ...
    async def _cleanup_connection(self, connection: WebSocketConnection) -> None:
        ...
        try:
            ...
            self._logger.info("WebSocket connection cleaned up", {
                "connection_id": connection.connection_id,
                "session_duration": (datetime.utcnow() - connection.connected_at).total_seconds()
            })
            
        except Exception as e:
            self._logger.error("Connection cleanup error", {
                "connection_id": connection.connection_id,
                "error": str(e)
            }, exception=e)
    ...
    async def broadcast_message(self, topic: str, message: Dict[str, Any]) -> int:
        ...
        self._logger.debug("Broadcast message sent", {
            "topic": topic,
