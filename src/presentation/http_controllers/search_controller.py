...
class SearchController(ContainerController):
    ...
    async def get_search_status(self, request_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Log success
            self._log_request_success("Search Status", {"request_id": request_id}, start_time)

            return self._create_success_response(result)

        except Exception as e:
            self._log_request_error(
                "Search Status", {"request_id": request_id if "request_id" in locals() else "unknown"}, start_time, e
            )
            return self._handle_exception(e)
