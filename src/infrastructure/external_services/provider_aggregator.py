...
class ProviderAggregator(IProviderAggregator):
    ...
    async def get_aggregated_offers(self, address: Address, 
        ...
        try:
            ...
            self._logger.info(f"Aggregated {len(unique_offers)} unique offers from {len(providers)} providers in {processing_time:.2f}s")
    ...
    async def get_parallel_offers(self, address: Address, 
        ...
        try:
            ...
            for provider in providers:
                task = self._get_provider_offers_with_timeout(
                    semaphore, provider, address, timeout_seconds
                )
    ...
    async def get_best_offers(self, address: Address, 
                             max_offers: int = 10,
                             sort_by: str = "price") -> List[ProviderOffer]:
    ...
    async def get_offers_by_connection_type(self, address: Address, 
                                           connection_types: List[str]) -> Dict[str, List[ProviderOffer]]:
        ...
        try:
            ...
            for connection_type in connection_types:
                filtered_offers = [
                    ...
                    if offer.connection_type.value.upper() == connection_type.upper()
                ]
    ...
    async def _get_provider_offers_with_semaphore(self, semaphore: asyncio.Semaphore, 
                                                 provider: IProviderService, 
                                                 address: Address) -> List[ProviderOffer]:
    ...
    async def _get_provider_offers_with_timeout(self, semaphore: asyncio.Semaphore,
                                               provider: IProviderService, 
                                               address: Address,
                                               timeout_seconds: int) -> List[ProviderOffer]:
        """Get offers from a provider with timeout."""
        async with semaphore:
            try:
                return await asyncio.wait_for(
                    provider.get_offers(address), 
