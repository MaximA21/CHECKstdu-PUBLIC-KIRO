...
class MockProviderService(IProviderService):
    ...
    def __init__(
        self,
        provider_name: str,
        provider_type: ProviderType = ProviderType.API_BASED,
    ...
    async def get_offers(self, address: Address) -> List[ProviderOffer]:
        ...
        for i in range(num_offers):
            ...
            offer = ProviderOffer(
                ...
                setup_fee_euros=(
                    Decimal(str(random.uniform(0, 99.99))).quantize(Decimal("0.01")) if random.random() > 0.5 else None
                ),
...
class MockPingPerfect(MockProviderService, IPingPerfect):
    ...
    async def get_signed_offers(self, address: Address) -> List[ProviderOffer]:
        ...
        # Sign the request
        request_data = {"address": address.full_address, "timestamp": datetime.utcnow().isoformat()}
