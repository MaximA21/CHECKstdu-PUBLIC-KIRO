...
class WebWunderAdapter(IWebWunder):
    ...
    async def _get_aggregated_response(self, address: Address) -> List[Dict[str, Any]]:
        ...
        return [
            {
                "connection_type": "DSL",
                "response_data": self._get_mock_soap_response("DSL")
            },
            {
                "connection_type": "Cable",
                "response_data": self._get_mock_soap_response("Cable")
            },
            {
                "connection_type": "Fiber",
                "response_data": self._get_mock_soap_response("Fiber")
            }
        ]
            ...
            for i, product_elem in enumerate(product_elements, 1):
                try:
                    ...
                    if product_info is not None:
                        ...
                        contract_duration = self._safe_int(self._get_xml_text(product_info, 'contractDurationInMonths', namespaces))
                        ...
                        if voucher_elem is not None:
                            ...
                            voucher_max_discount = self._safe_int(self._get_xml_text(voucher_elem, 'maxDiscountInCent', namespaces))
    ...
    def _calculate_metadata(self, aggregated_response: List[Dict], offers: List[ProviderOffer]) -> Dict[str, Any]:
        ...
        return {
            ...
            "connection_types_with_offers": len(set(offer.get_feature("connection_type_source") for offer in offers if offer.has_feature("connection_type_source"))),
