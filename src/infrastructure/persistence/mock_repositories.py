...
class MockSearchResultRepository(ISearchResultRepository):
    ...
    async def save_result(self, result: SearchResult) -> str:
        ...
        self._results_by_address[address_key] = [
            r for r in self._results_by_address[address_key] 
            if r.request_id != result.request_id
        ]
        self._results_by_address[address_key].append(deepcopy(result))
        
        # Sort by timestamp (newest first)
        self._results_by_address[address_key].sort(
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "expired_results": len([
                r for r in self._results_by_token.values() 
...
class MockConnectionRepository(IConnectionRepository):
    ...
    async def get_active_connections(self) -> List[ConnectionSession]:
        ...
        active_sessions = [
            session for session in self._connections.values()
...
class MockProviderOfferRepository(IProviderOfferRepository):
    ...
    async def get_cached_offers(self, address: Address, provider_name: str, 
                               max_age_hours: int = 24) -> Optional[List[ProviderOffer]]:
    ...
    async def invalidate_cache(self, address: Address, provider_name: Optional[str] = None) -> bool:
        ...
        if provider_name:
            ...
            address_prefix = f"{address.normalized_postal_code}_{address.house_number.strip().lower()}_{address.city.strip().lower()}:"
            keys_to_remove = [
                key for key in self._cached_offers.keys()
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "unique_providers": len(set(
                data["provider_name"] for data in self._cached_offers.values()
            ))
        }