...
from src.infrastructure.persistence.mock_repositories import (
    MockSearchResultRepository, MockConnectionRepository
)
...
class ServiceSelector:
    ...
    def is_mock_environment(self) -> bool:
        ...
        return (
            self.config.database.provider == DatabaseProvider.MOCK and
            self.config.messaging.provider == MessagingProvider.MOCK
        )
    
    def is_aws_environment(self) -> bool:
        """Check if running in AWS environment."""
        return (
            self.config.database.provider == DatabaseProvider.AWS_DYNAMODB or
            self.config.messaging.provider == MessagingProvider.AWS_SQS or
            self.config.logging.provider == LoggingProvider.AWS_CLOUDWATCH
        )
    ...
    def get_environment_summary(self) -> Dict[str, str]:
        ...
        return {
            ...
            "providers_enabled": str(len([
                p for p in self.config.provider_configs.values() if p.enabled
            ]))
        }