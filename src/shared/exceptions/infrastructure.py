...
class DatabaseConnectionException(RetryableException):
    ...
    def __init__(self, message: str, database_name: Optional[str] = None, connection_string: Optional[str] = None, **kwargs):
...
class MessageQueueException(RetryableException):
    ...
    def __init__(self, message: str, queue_name: Optional[str] = None, operation: Optional[str] = None, **kwargs):
        """Initialize message queue exception."""
        context = kwargs.get("context", ErrorContext())
        context.additional_data.update({"queue_name": queue_name, "operation": operation})
...
class StorageException(RetryableException):
    ...
    def __init__(self, message: str, operation: Optional[str] = None, resource_id: Optional[str] = None, **kwargs):
        """Initialize storage exception."""
        context = kwargs.get("context", ErrorContext())
        context.additional_data.update({"operation": operation, "resource_id": resource_id})
...
class ConnectionManagerException(InfrastructureException):
    ...
    def __init__(self, message: str, connection_id: Optional[str] = None, operation: Optional[str] = None, **kwargs):
        """Initialize connection manager exception."""
        context = kwargs.get("context", ErrorContext())
        context.additional_data.update({"connection_id": connection_id, "operation": operation})

        super().__init__(message, severity=ErrorSeverity.HIGH, category=ErrorCategory.NETWORK, context=context, **kwargs)
...
class LoggingException(InfrastructureException):
    ...
    def __init__(self, message: str, logger_name: Optional[str] = None, log_level: Optional[str] = None, **kwargs):
        """Initialize logging exception."""
        context = kwargs.get("context", ErrorContext())
        context.additional_data.update({"logger_name": logger_name, "log_level": log_level})

        super().__init__(
            message, severity=ErrorSeverity.MEDIUM, category=ErrorCategory.SYSTEM, context=context, recoverable=True, **kwargs
        )
