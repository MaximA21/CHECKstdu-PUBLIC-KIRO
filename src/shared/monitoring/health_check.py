        ...
        try:
            ...
            if self.logger:
                self.logger.warning(
                    ...
                    {'component': self.name, 'timeout': self.timeout_seconds}
                )
...
class DatabaseHealthCheck(HealthCheck):
    ...
    def __init__(
        self,
        name: str,
        connection_test_func: Callable[[], Any],
        **kwargs
    ):
...
class ExternalServiceHealthCheck(HealthCheck):
    ...
    def __init__(
        self,
        name: str,
        service_test_func: Callable[[], Any],
...
class HealthCheckRegistry:
    ...
    def unregister(self, name: str) -> bool:
        ...
        if name in self.health_checks:
            ...
            if self.logger:
                self.logger.info(
                    f"Health check unregistered: {name}",
                    {'component': name}
                )
            return True
        return False
    ...
    def get_unhealthy_components(self) -> List[ComponentHealth]:
        ...
        return [
            health for health in self.last_results.values()
