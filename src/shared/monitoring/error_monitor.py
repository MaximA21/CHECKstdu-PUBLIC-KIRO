...
class ErrorMonitor:
    ...
    def record_error(self, error: Exception, context: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(error, BaseApplicationException):
            ...
            error_record.update(
                {"severity": ErrorSeverity.HIGH.value, "category": ErrorCategory.SYSTEM.value, "recoverable": False}
            )
        ...
        # Log the error
        if self.logger:
            self.logger.error(f"Error recorded: {error_record['error_type']}", error_record)
    ...
    def _update_metrics(self, error_record: Dict[str, Any]) -> None:
        ...
        self.metrics.errors_by_component[component] = self.metrics.errors_by_component.get(component, 0) + 1
    ...
    def _check_alert_conditions(self, error_record: Dict[str, Any]) -> None:
        ...
        high_errors = [error for error in recent_errors if error.get("severity") == ErrorSeverity.HIGH.value]
        ...
        medium_errors = [error for error in recent_errors if error.get("severity") == ErrorSeverity.MEDIUM.value]
    ...
    def _trigger_alert(self, alert_type: str, severity: AlertSeverity, title: str, message: str, error_count: int) -> None:
        ...
        # Log alert
        if self.logger:
            self.logger.error(f"Alert triggered: {title}", alert.to_dict())
        ...
        for callback in self.alert_callbacks:
            try:
                ...
                if self.logger:
                    self.logger.error(
                        f"Error in alert callback: {str(e)}", {"callback": callback.__name__, "alert_id": alert_id}
                    )
    ...
    def get_error_history(self, limit: Optional[int] = None, since: Optional[datetime] = None) -> List[Dict[str, Any]]:
