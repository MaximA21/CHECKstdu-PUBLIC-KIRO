...
class RetryHandler(ABC):
    ...
    def __init__(self, max_retries: int = 3, base_delay: float = 1.0, logger: Optional[ILogger] = None):
    ...
    async def execute_with_retry(self, func: Callable, *args, **kwargs) -> Any:
            ...
            try:
                ...
                if self.logger:
                    self.logger.warning(
                        ...
                        {"error": str(e), "attempt": attempt + 1, "delay": delay, "function": func.__name__},
                    )
        ...
        if self.logger:
            self.logger.error(
                ...
                {"error": str(last_error), "max_retries": self.max_retries, "function": func.__name__},
            )
...
class GracefulDegradationHandler:
    ...
    def __init__(self, fallback_strategies: Optional[Dict[str, Callable]] = None, logger: Optional[ILogger] = None):
    ...
    def mark_service_unhealthy(self, service_name: str, error: Exception) -> None:
        ...
        self.service_health[service_name] = {"healthy": False, "last_error": str(error), "timestamp": time.time()}

        if self.logger:
            self.logger.warning(
                f"Service {service_name} marked as unhealthy",
                {"service": service_name, "error": str(error), "timestamp": time.time()},
            )

    def mark_service_healthy(self, service_name: str) -> None:
        """Mark a service as healthy."""
        self.service_health[service_name] = {"healthy": True, "last_error": None, "timestamp": time.time()}

        if self.logger:
            self.logger.info(f"Service {service_name} marked as healthy", {"service": service_name, "timestamp": time.time()})
            ...
            if service_name in self.fallback_strategies:
                if self.logger:
                    self.logger.info(
                        ...
                        {"service": service_name, "error": str(e), "fallback_available": True},
                    )
                ...
                if self.logger:
                    self.logger.error(
                        ...
                        {"service": service_name, "error": str(e), "fallback_available": False},
                    )
                raise e
...
def with_fallback(service_name: str, fallback_func: Callable):
    ...
    def decorator(func: Callable) -> Callable:
        @wraps(func)
        async def async_wrapper(*args, **kwargs):
            degradation_handler = GracefulDegradationHandler({service_name: fallback_func})
            return await degradation_handler.execute_with_fallback(service_name, func, *args, **kwargs)

        @wraps(func)
        def sync_wrapper(*args, **kwargs):
            degradation_handler = GracefulDegradationHandler({service_name: fallback_func})
            return asyncio.run(degradation_handler.execute_with_fallback(service_name, func, *args, **kwargs))
