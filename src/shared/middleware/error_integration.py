...
class ComprehensiveErrorHandler:
    ...
    def _handle_error_alert(self, alert) -> None:
        ...
        if self.logger:
            self.logger.error(
                f"Error alert triggered: {alert.title}",
                alert_data
            )
            ...
            try:
                ...
                if self.logger:
                    self.logger.error(
                        ...
                        {'callback': callback.__name__, 'alert_id': alert.alert_id}
                    )
    ...
    def get_system_health(self) -> Dict[str, Any]:
        ...
        if self.circuit_breaker_registry:
            health_data['circuit_breakers'] = {
                name: state.value
                for name, state in self.circuit_breaker_registry.get_all_states().items()
            }
