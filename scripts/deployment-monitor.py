...
class DeploymentMonitor:
        ...
        for func_name in functions:
            ...
            try:
                ...
                if memory_response['Datapoints']:
                    memory_util = sum(point['Average'] for point in memory_response['Datapoints']) / len(memory_response['Datapoints'])
    ...
    def _generate_monitoring_summary(self, results: Dict):
        ...
        if results['snapshots']:
            ...
            avg_error_rate = sum(s['lambda_metrics']['summary']['error_rate'] for s in results['snapshots']) / len(results['snapshots'])
            avg_duration = sum(s['lambda_metrics']['summary']['avg_duration'] for s in results['snapshots']) / len(results['snapshots'])
...
def main():
    ...
    parser.add_argument('--response-threshold', type=int, default=5000,
                       help='Response time threshold in ms (default: 5000)')
