...
class TrafficDistributionValidator:
        ...
        try:
            async with websockets.connect(websocket_url) as websocket:
                ...
                test_message = {
                    "action": "search",
                    "data": {
                        "address": f"Test Address {connection_id}",
                        "providers": ["test"]
                    }
                }
    ...
    def validate_canary_deployment(self, api_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                stage_response = self.apigateway.get_stage(
                    restApiId=api_id,
                    stageName='prod'
                )
    ...
    async def run_comprehensive_validation(self, rest_api_name: str, websocket_api_name: str) -> Dict[str, Any]:
        ...
        if websocket_endpoint:
            success_criteria.append(
                results.get('websocket_traffic_distribution', {}).get('success', False)
            )
...
def main():
    ...
    parser.add_argument('--rest-api-name', default='provider-comparison-dev-rest-api', 
                       help='REST API Gateway name')
    parser.add_argument('--websocket-api-name', default='provider-comparison-dev-websocket-api', 
                       help='WebSocket API Gateway name')
    parser.add_argument('--region', default='eu-central-1', help='AWS region')
    parser.add_argument('--output', help='Output file for results (JSON format)')
    parser.add_argument('--rest-requests', type=int, default=50, 
                       help='Number of REST API requests for distribution testing')
    parser.add_argument('--websocket-connections', type=int, default=20, 
                       help='Number of WebSocket connections for distribution testing')
