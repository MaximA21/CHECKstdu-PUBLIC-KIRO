...
class DeploymentValidator:
    ...
    def validate_api_endpoints(self, base_url: str) -> Dict[str, Any]:
        ...
        test_results = {"status": "unknown", "endpoints": {}, "response_times": [], "errors": []}
        ...
        for endpoint in endpoints:
            ...
            try:
                ...
                if endpoint["method"] == "GET":
                    response = requests.get(f"{base_url}{endpoint['path']}", timeout=self.config["timeouts"]["api_test"])
                ...
                test_results["endpoints"][endpoint_name] = {"status": "error", "error": str(e)}
    ...
    def validate_websocket_connection(self, ws_url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            def on_open(ws):
                ...
                # Send test message
                test_message = json.dumps({"action": "search", "data": {"query": "test", "location": "Berlin"}})
    ...
    def validate_lambda_functions(self) -> Dict[str, Any]:
        ...
        test_results = {"status": "unknown", "functions": {}, "errors": []}
        ...
        for func_name in functions:
            ...
            try:
                ...
                is_healthy = (
                    ...
                    and runtime.startswith("python3")
                    and memory >= 128
                    and timeout >= 30
                )
                    ...
                    if func_name not in ["connect_handler", "disconnect_handler"]:
                        try:
                            ...
                            invoke_response = self.lambda_client.invoke(
                                FunctionName=full_name, InvocationType="RequestResponse", Payload=json.dumps(test_payload)
                            )
                ...
                test_results["functions"][func_name] = {"status": "error", "error": str(e)}
    ...
    def validate_data_persistence(self) -> Dict[str, Any]:
        ...
        test_results = {"status": "unknown", "tables": {}, "errors": []}
        ...
        for table_name in expected_tables:
            ...
            try:
                ...
                if table_status == "ACTIVE":
                    ...
                    # Read test item
                    get_response = self.dynamodb_client.get_item(TableName=table_name, Key={"id": test_item["id"]})

                    if "Item" in get_response:
                        # Clean up test item
                        self.dynamodb_client.delete_item(TableName=table_name, Key={"id": test_item["id"]})

                        test_results["tables"][table_name] = {"status": "passed", "table_status": table_status}
                        success_count += 1
                        print(f"    ✅ {table_name}: {table_status}")
                    else:
                        test_results["tables"][table_name] = {"status": "failed", "error": "Read operation failed"}
                ...
                test_results["tables"][table_name] = {"status": "error", "error": str(e)}
    ...
    def run_load_test(self, api_url: str, duration: int = 60) -> Dict[str, Any]:
        ...
        def make_request():
            ...
            try:
                start_time = time.time()
                response = requests.post(
                    f"{api_url}/api/search", json={"query": "load test", "location": "Berlin"}, timeout=10
                )
        ...
        success_rate = (
            (test_results["successful_requests"] / test_results["total_requests"]) * 100
        ...
        # Determine status
        if (
            success_rate >= self.config["thresholds"]["min_success_rate"]
            and test_results["avg_response_time"] < self.config["thresholds"]["max_response_time"]
        ):
            test_results["status"] = "passed"
        else:
            test_results["status"] = "failed"
    ...
    def run_comprehensive_validation(
        self, api_url: str = None, ws_url: str = None, include_load_test: bool = False
    ) -> Tuple[bool, Dict]:
...
def main():
    ...
    parser.add_argument("environment", choices=["staging", "production"], help="Environment to validate")
    ...
    try:
        ...
        is_valid, results = validator.run_comprehensive_validation(
            api_url=args.api_url, ws_url=args.ws_url, include_load_test=args.include_load_test
        )
