...
class CanaryDeploymentManager:
    ...
    def get_stage_info(self, api_id: str, stage_name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            response = self.apigateway.get_stage(restApiId=api_id, stageName=stage_name)
            return response
        except (ClientError, BotoCoreError) as e:
    ...
    def update_canary_traffic(self, api_id: str, stage_name: str, percent_traffic: int) -> bool:
        ...
        try:
            ...
            self.apigateway.update_stage(
                ...
                patchOps=[{"op": "replace", "path": "/variables/traffic_split", "value": str(percent_traffic)}],
            )
    ...
    def check_health(self, api_name: str, stage_name: str) -> bool:
        ...
        print(
            f"✅ Health check passed - Error rate: {metrics.get('error_rate', 0):.2f}%, Latency: {metrics.get('avg_latency', 0):.2f}ms"
        )
        return True

    def gradual_traffic_shift(
        self, api_name: str, stage_name: str, target_percent: int, step_size: int = 10, wait_minutes: int = 5
    ) -> bool:
