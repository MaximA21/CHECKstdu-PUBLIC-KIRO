...
class SecurityValidator:
        ...
        try:
            ...
            for key in keys['Keys']:
                ...
                try:
                    ...
                    if (key_metadata['KeyManager'] == 'CUSTOMER' and 
                        key_metadata.get('Description', '').find(self.project_name) != -1):
                        custom_keys.append({
                            'key_id': key_id,
            ...
            if len(custom_keys) == 0:
                ...
                results['details'].append(f"⚠️ Found {len(custom_keys)} custom KMS keys - consider using AWS managed keys for cost savings")
    ...
    def validate_lambda_iam_policies(self) -> Dict[str, Any]:
        ...
        try:
            ...
            project_functions = [
                ...
                if f['FunctionName'].startswith(f"{self.project_name}-{self.environment}")
            ]
                ...
                try:
                    ...
                    try:
                        ...
                        if len(violations) == 0:
                            ...
                            results['policy_violations'].extend([
                                f"{function_name}: {violation}" for violation in violations
                            ])
            ...
            if results['functions_checked'] == 0:
                ...
                results['status'] = 'compliant'
                results['details'].append(f"✅ All {results['functions_checked']} Lambda functions have appropriate IAM policies")
            else:
                results['status'] = 'non_compliant'
                results['details'].append(f"❌ {results['functions_checked'] - results['compliant_functions']} functions have policy violations")
    ...
    def validate_s3_security(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Determine overall status
            if (results['public_access_blocked'] and results['encryption_enabled'] and 
