...
class HealthChecker:
    ...
    def run_comprehensive_health_check(
        self, api_endpoint: Optional[str] = None, ws_endpoint: Optional[str] = None, check_metrics: bool = True
    ) -> Tuple[bool, Dict]:
        ...
        # Overall health
        overall_healthy = lambda_healthy and api_healthy and ws_healthy and error_rates_healthy and metrics_healthy_overall
...
def main():
    ...
    parser.add_argument("environment", choices=["staging", "production"], help="Environment to check")
    ...
    parser.add_argument("--ws-endpoint", help="WebSocket Gateway endpoint URL")
    parser.add_argument("--no-metrics", action="store_true", help="Skip CloudWatch metrics check (faster)")
    ...
    try:
        ...
        is_healthy, results = checker.run_comprehensive_health_check(
            api_endpoint=args.api_endpoint, ws_endpoint=args.ws_endpoint, check_metrics=not args.no_metrics
        )
