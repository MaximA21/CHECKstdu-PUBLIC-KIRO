        ...
        try:
            ...
            if self.lambda_client is None:
                ...
                if active_functions >= len(essential_functions):
                    ...
                    print(f"  ⚠️ Lambda functions not deployed yet ({not_found_functions}/{len(essential_functions)} not found)")
...
def main():
    ...
    parser.add_argument('--api-endpoint', 
                       help='API Gateway endpoint URL for connectivity tests')
    ...
    try:
        # Run streamlined tests
        is_valid, results = tester.run_streamlined_deployment_tests(
