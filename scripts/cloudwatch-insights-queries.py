...
class CloudWatchInsightsRunner:
    ...
    def run_query(self, query: str, log_groups: List[str], 
...
def main():
    ...
    parser.add_argument('--query-type', choices=[
        'connections', 'errors', 'performance', 'traffic', 'limits'
    ], required=True, help='Type of query to run')
        ...
        if result.get('status') == 'success':
            ...
            if results:
                ...
                for i, row in enumerate(results[:10]):  # Show top 10
                    print(f"{i+1}. {' | '.join([f'{field[\"field\"]}: {field[\"value\"]}' for field in row])}")
            else:
