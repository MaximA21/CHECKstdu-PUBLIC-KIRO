    ...
    def identify_expensive_resources(self) -> Dict[str, List[Dict]]:
                        ...
                        expensive_resources["long_retention_logs"].append(
                            {
                                ...
                                "estimated_monthly_cost": (log_group.get("storedBytes", 0) / 1024**3)
                                * 0.50,  # ~$0.50/GB/month
                            }
                        )
        except Exception as e:
        ...
        try:
            traces = self.xray.get_trace_summaries(
                TimeRangeType="TimeRangeByStartTime", StartTime=boto3.session.Session().region_name
            )
    ...
    def reduce_log_retention(self, log_groups: List[Dict], target_retention: int = 7) -> List[str]:
        ...
        for log_group in log_groups:
            ...
            if self.dry_run:
                ...
                try:
                    self.logs.put_retention_policy(logGroupName=log_group_name, retentionInDays=target_retention)
                    actions.append(
